...
class Store {
 public:
  ...
  void assignProject(int id, int projectId) { sqlite3_stmt* s=prepare("UPDATE tasks SET project_id=? WHERE id=?"); if(projectId)sqlite3_bind_int(s,1,projectId);else sqlite3_bind_null(s,1); sqlite3_bind_int(s,2,id); sqlite3_step(s); sqlite3_finalize(s); }
...
class App {
 ...
 private:
  ...
  void moveToProject(const Task& t) { auto ps=store_.projects(); int rows,cols; getmaxyx(stdscr,rows,cols); WINDOW*w=newwin(10,58,(rows-10)/2,(cols-58)/2); box(w,0,0); mvwprintw(w,1,2,"MOVE TO PROJECT"); int y=3; for(auto&p:ps){ mvwprintw(w,y++,2,"• %s",p.name.c_str()); if(y==7)break; } mvwprintw(w,8,2,"Project name: "); wrefresh(w); echo(); curs_set(1); char name[180]{}; wgetnstr(w,name,170); noecho(); curs_set(0); for(auto&p:ps)if(p.name==name)store_.assignProject(t.id,p.id); delwin(w); }
  void moveTask(const Task&t) { int rows,cols; getmaxyx(stdscr,rows,cols); WINDOW*w=newwin(10,42,(rows-10)/2,(cols-42)/2); box(w,0,0); mvwprintw(w,1,2,"MOVE TASK"); for(int i=0;i<5;++i)mvwprintw(w,3+i,2,"%d  %s",i+1,views_[i].c_str()); mvwprintw(w,8,2,"p  Project"); wrefresh(w); int k=wgetch(w); if(k>='1'&&k<='5')store_.move(t.id,views_[k-'1']); delwin(w); if(k=='p')moveToProject(t); }
  void help() { int r,c;getmaxyx(stdscr,r,c);WINDOW*w=newwin(12,58,(r-12)/2,(c-58)/2);box(w,0,0);mvwprintw(w,1,2,"KEYBOARD");mvwprintw(w,3,2,"j/k ↑/↓  navigate     h/l  switch lists");mvwprintw(w,5,2,"n new task   p project    Enter details");mvwprintw(w,7,2,"e edit  x complete  m move (then p project)");mvwprintw(w,9,2,"b sidebar  q quit  • JetBrainsMono Nerd Font icons");wrefresh(w);wgetch(w);delwin(w);}
